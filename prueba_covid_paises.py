...
def num_parser(etiqueta):
    ...
    if etiqueta == "''" or etiqueta == '':
        ...
        try:
            return int(etiqueta.replace(",", ""))
        except ValueError:
            try:
                pattern = r'^([0-9]+).*'
                num_et = re.findall(pattern, etiqueta)[0]
                # Debug
                # print(num_et)
                return int(num_et)
...
fecha = soup.find("div", class_="content-inner").find_all("div")[1].text
...
for fila in filas:
    ...
    if a is not None and pais not in lista_paises:
        ...
        fecha = soup.find("div",
                          class_="content-inner").find_all("div")[1].text
        ...
        df = creafila(columnas, [pais, 'Total', casos,
                                 fallecimientos, recuperados,
                                 casos - fallecimientos - recuperados,
                                 fallecimientos + recuperados,
                                 fecha_act])
        df_act = df_act.append(df, ignore_index=True)
        # Cerramos conexiones
